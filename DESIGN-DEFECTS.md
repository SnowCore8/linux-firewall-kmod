...
9. **Netlink 发送成功 ≠ 内核成功** — `sendto` 后立即提交缓存/历史；`CmdResult` 仅打日志。需请求-ACK 状态机。
10. **守护进程接收未校验发送方** — `recv` 不验 `sockaddr_nl`/凭据，本地可伪造事件。
11. **封禁缓存无周期性对账** — 仅启动全量同步且只增不删；过期清理挂在 GET 副作用。
