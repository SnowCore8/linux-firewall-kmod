...
static int generate_metrics(char *buf, size_t buf_size) {
  ...
  return snprintf(
      ...
      "\n",
      kernel_banned, kernel_total_bans, kernel_total_unbans,
      kernel_whitelist_count, d_lines_parsed, d_ips_extracted, d_ips_banned,
      d_failed_attempts, d_config_reloads, d_inotify_events, d_log_rotations,
      d_lines_skipped, d_regex_matches, (long)uptime);
}
