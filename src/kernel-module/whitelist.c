...
int add_whitelist_entry(struct firewall_info *fw, __be32 ip, __be32 mask,
  ...
  FW_DEBUG(1, "ENTRY: add_whitelist_entry(ip=%pI4, mask=%pI4, dev=%s)", &ip,
           &mask, dev_name ?: "null");
  ...
  FW_DEBUG(2, "Attempting to add whitelist entry for %pI4/%d", &normalized_ip,
           inet_mask_len(mask));

  /* 在锁外分配内存 */
  new_entry = kmalloc(sizeof(*new_entry), GFP_KERNEL);
  if (!new_entry) {
    FW_DEBUG(1, "Failed to allocate memory for whitelist entry for IP %pI4",
             &normalized_ip);
  ...
  hash_for_each_possible(fw->whitelist_table, tmp_entry, hash, normalized_ip) {
    if (compare_ips(tmp_entry->ip, normalized_ip) && tmp_entry->mask == mask) {
  ...
  fw_pr_info("Whitelisted %pI4/%d on %s", &normalized_ip, inet_mask_len(mask),
             dev_name ?: "unknown");
