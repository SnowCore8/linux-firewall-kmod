...
**关键优势**：
- 写锁持有时间从 ~340 行降至 ~50 行
- 配置解析在锁外执行，不阻塞读取线程
- 原子指针交换保证配置一致性

**工作原理**：

传统热重载在持有写锁的情况下完成配置解析、验证和交换全过程，导致读取线程在此期间被阻塞。v2.1 引入双缓冲机制，将耗时的配置解析操作移至锁外执行：

1. **锁外阶段**：收到 SIGHUP 信号后，先在无锁状态下将新配置解析到临时结构体，完成所有格式校验和语义检查
2. **临界区阶段**：仅获取写锁后执行指针原子交换和运行时状态迁移，将写锁持有时间压缩至最低
3. **清理阶段**：释放写锁后，通过 RCU 或延迟队列安全回收旧配置内存

这种设计大幅降低了热重载期间的读取延迟抖动，特别适合高并发场景下的配置更新。
