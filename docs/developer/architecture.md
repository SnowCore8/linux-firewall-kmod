...
| 参数 | 类型 | 默认值 | 权限 | 说明 |
|------|------|--------|------|------|
| `fw_ban_time` | uint | 600 | 0644 | 默认封禁时长（秒） |
| `fw_max_bans_per_second` | uint | 200 | 0644 | 每秒最大封禁次数（洪泛保护） |
| `state_file` | charp | `/var/lib/firewall/state` | 0444 | 状态文件路径（只读） |

### 3.5 procfs 接口

内核模块通过 `/proc/firewall/` 目录提供用户态交互接口，包括封禁管理、白名单管理、运行时配置和统计信息。

详细的接口操作说明、命令示例和限制条件，请参考 [运维操作手册 → procfs 接口](../user-guide/operations.md#2-procfs-接口)。
...
| 封禁查找 | O(1) | 哈希表查找 |
| 封禁插入 | O(1) | 锁外预分配 + 锁内插入 |
| 白名单精确匹配 | O(1) | 两阶段策略：哈希查找 /32 条目 |
| 白名单子网匹配 | O(n) | 跳过 /32 条目优化，遍历子网条目 |
| SQLite 操作 | O(1) | 缓存 prepared statements |
...
| 输入验证 | IP 格式检查、IP 长度检查、控制字符过滤、count 验证、路径遍历防护、URL 编码检测 |
| 并发安全 | RCU + spinlock + READ_ONCE/WRITE_ONCE 全面配对、双缓冲配置交换 |
