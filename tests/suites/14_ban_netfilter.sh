...
assert_file_contains "$PROC_BANS" "$TEST_NETFILTER_IP" "IP 出现在内核 ban_table 中"
...
# 14.5 发送测试数据包
fw_subsection "netfilter 数据包丢弃测试"
ping -c 3 -W 1 $TEST_NETFILTER_IP > /dev/null 2>&1 || true
sleep 1

AFTER_DROPPED=$(grep "packets_dropped" $PROC_STATS | awk '{print $2}')
DROPPED_DIFF=$((AFTER_DROPPED - BEFORE_DROPPED))

assert_true "[[ $DROPPED_DIFF -gt 0 ]]" "netfilter 钩子丢弃 $DROPPED_DIFF 个数据包"
