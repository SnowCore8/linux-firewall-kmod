...
for i in $(seq 1 50); do
    echo "203.0.113.$((i%256))" > "$PROC_ADD_BAN" 2>/dev/null || true
done
sleep 0.3

# 检查模块是否仍响应
assert_true "[[ -r '$PROC_BAN_LIST' ]]" "大量操作后模块仍响应"

# 清理
for i in $(seq 1 50); do
    echo "203.0.113.$((i%256))" > "$PROC_REMOVE_BAN" 2>/dev/null || true
done
...
for i in $(seq 1 200); do
    if echo "10.0.$((i/256)).$((i%256))" > "$PROC_ADD_BAN" 2>/dev/null; then
...
# 分批清理
for i in $(seq 1 200); do
    echo "10.0.$((i/256)).$((i%256))" > "$PROC_REMOVE_BAN" 2>/dev/null || true
    if (( i % 50 == 0 )); then
        sleep 0.05
    fi
done
