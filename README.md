...
Firewall 是一个 Linux 内核模块版本的 fail2ban，将封禁逻辑从用户空间移至内核空间，使用 netfilter 框架在数据包级别进行实时 IP 封禁，具有更低的延迟和更高的性能。守护进程用 Rust 实现（v2.2.0 起从 C 翻译），二进制 3.8MB stripped，103 项集成测试全部通过。
...
- ✅ **Rust 守护进程（v2.2.0+）** — 58 个源文件，3.8MB stripped 二进制，行为与 C 版严格等价
- ✅ **正则解析** — 支持命名捕获组提取 IP
...
- ✅ **安全加固** — 整数溢出防护、Use-After-Free 修复、RCU 一致性增强、20 个 unsafe 块全部带 `// SAFETY:` 注释
- ✅ **性能优化** — 哈希表容量 4096、白名单两阶段匹配、LTO 编译优化
- ✅ **代码质量** — 88 单元测试 + 103 集成测试 100% 通过，CI 三 job 全绿
